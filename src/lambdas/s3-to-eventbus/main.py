...
def send_events(events: List):
	...
	for i in range(0, len(events), 10):
		events.put_events(Entries=events[i:i + 10])
...
def s3_to_events(
	...
	return [{"Time": datetime.now(), "Source": source, "Resources": [s3_record.s3.get_object.key],
	         "EventBusName": event_bus_name, "DetailType": "s3", "Detail": json.dumps(item), } for item in raw_data]
